...
- `features/herdr-kickoff-automation.md` — herdr `herdr-launch.sh`: `launch` (`/kickoff` + `/adopt`) + `resume` (`/continue <task>` reopens an `/exit`-closed tab); feature-detected dual contract (legacy placement vs 0.7.5+ `agent start --kind --pane` into a pre-created tab), registry-declared transport, bounded busy-retry, rollback vs `blocked=unverified`, why a dead wrapper is detected from process state and never from terminal text
