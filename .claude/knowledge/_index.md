...
- `features/herdr-close-automation.md` — `/close` in herdr: cwd-tab teardown, plugin SessionEnd hook, the one TUI-exit primitive, detached self-exit onto idle, Manager close-delegation
...
- `features/task-archiving-on-close.md` — `/close` archives (not deletes) the task file; adaptive commit + ff-push to main; per-repo `.claude/work-system-close-autocommit` opt-in skips the ask; `--note-file` keeps an unsaved-report summary in the stamp
...
- `features/insights-lifecycle-producers.md` — work-system's `/continue` handoff + `/close` reports: absent vs. installed-but-unusable as distinct exit codes, report between merge gate and removal and never a gate, idempotency from the store itself, archived-task-file fallback with no second store, trigger ≠ task_status, contract path from the helper, prose ordering guarded by an assertion test; plus three shell traps (subshell array append, `${arr[@]/pat}`, `--date=format:` timezone)
- `features/insights-report-store.md` — insights.py as sole report write/read path: main-checkout identity, strict Fact shapes, link()-based no-overwrite publish, refuse-not-chmod store dirs, file drafts not heredocs, fixpoint redaction exempting identifiers, anchored URL parse, skeleton that fails until filled
