...
- **Worktree-aware secret denies**: in a linked worktree the deny globs also cover the **main checkout's** root (via absolute `--path-format=absolute --git-common-dir`) — untracked `.env`/`data/` live there, not in the worktree. The repo's own `.git/config` is deliberately **left readable** (git treats an EPERM on it as fatal, which would break the externals' git-based exploration); a repo-config-embedded token is an accepted, documented residual.
