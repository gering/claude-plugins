...
- A failed kimi seed is now a hard stop: it prints a machine-readable marker, states TASK.md was not started, and exits with the seed's own code — no keypress wait and no empty `kimi -c --auto` session that looks like a healthy worker. The marker is assembled at runtime, because a pane echoes the command it was given and the literal in the script text made a *healthy* launch roll itself back (found in live 0.8 testing).
