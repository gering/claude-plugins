...
- `agent-registry.sh resolve` now declares the transport per entry: `herdr_mode=agent-start` + `herdr_kind` for claude/codex/grok (argv[0] must equal the kind, is dropped, and every remaining argument keeps its order and boundaries), `herdr_mode=pane-run` + `herdr_marker=` for kimi's two-phase wrapper. Unknown transport metadata fails closed before anything is created. The metadata is generic so a dynamically-registered wrapper entry needs no launcher change.
...
- A failed kimi seed is now a hard stop: it prints a machine-readable marker, states TASK.md was not started, and exits with the seed's own code — no keypress wait and no empty `kimi -c --auto` session that looks like a healthy worker. The launcher treats that marker as a definitive failure and rolls the tab back, but only when it carries **this launch's random seed token**: a pane's contents are worker-controlled (it echoes the command it was given, and the seed reads repository files — this repo's own TASK.md, CHANGELOG and tests all name the marker), so a bare constant would tear down healthy workers. Found live in 0.8 testing, where the echoed command alone rolled a *healthy* launch back after two seconds.
- The marker is checked before the kind match in every poll, since herdr detects the wrapper's CLI within seconds of the seed starting and both signals can be true at once. What it catches, stated plainly: a seed that dies **before** the launch is confirmed (expired auth, an unconfigured model). A seed that fails minutes later cannot be caught at launch time — the seed phase *is* the task work — and is an ordinary worker crash, not a bad launch.
