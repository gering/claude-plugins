...
| `codex` | external reviewer | `codex exec --output-schema` in a read-only sandbox, pinned to model `gpt-5.6-terra` at `xhigh` reasoning effort; auth via `codex login status` |
