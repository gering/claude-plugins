...
check("backtick escaped", sp("a`b") == "a\\`b")
...
# --- sanitize_code: inert code span, only backtick/pipe/newline matter ------ #
check("code wraps", sc("file.sh:42") == "`file.sh:42`")
check("code strips backtick", "`" == sc("a`b`c")[0] and sc("a`b`c") == "`abc`")
check("code escapes pipe", sc("a|b") == "`a\\|b`")
...
check("row notiz pipe escaped in body", "pipe\\|here" in body)
...
check("6-col omits Source", "Source" not in body6.split("\n")[2])
...
# --- bad input -> exit 2, no body ------------------------------------------ #
with tempfile.NamedTemporaryFile("w", suffix=".json", delete=False) as f:
    f.write("not json")
    bad = f.name
p = subprocess.run(["python3", str(SCRIPT), "build", "--input", bad],
                   capture_output=True, text=True)
check("bad json -> exit 2", p.returncode == 2)
Path(bad).unlink()
