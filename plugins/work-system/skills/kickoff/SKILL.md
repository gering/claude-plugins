...
12. **Launch the worktree session** — automate it inside herdr, otherwise show
    ...
    **tab label** itself, mirroring the `[ws …]` statusline. This `LABEL` names
    the herdr agent, the tab, and the Claude session — but the glyph rides only
    on the tab label (what the sidebar renders); the agent and session names
    stay plain. The `task/<task-name>` branch is unchanged, so
