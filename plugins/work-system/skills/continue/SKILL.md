...
2. **Reopen + resume** — automate inside herdr, otherwise show the manual block.
   ...
   **a) Inside herdr — reopen the tab and resume via the shared helper:**
   ...
   # /kickoff — pass it PLAIN; the helper prefixes the task's state glyph (○ ● ◇ ◆ ✓)
   # onto the tab LABEL itself (agent + session names stay plain).
   ...
   Always include the non-claude worker line: the reopen can't tell the worker, so the
   caveat must be inline, not left to the top-of-section note.
