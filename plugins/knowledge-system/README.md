...
**What it does.** `createdAt` and `updatedAt` can be derived from `git log` when missing. Dates use ISO-8601 date-only (`YYYY-MM-DD`, UTC) — no time-of-day, no timezone confusion, diff-friendly.

**Why it matters.** Nobody remembers to update an `updatedAt` field. The git history does. `/reindex` and `/curate` fill it in for you, so the field is trustworthy when `/query` ranks results by recency.
...
pluginVersion: 1.3.0                 # knowledge-system version at last write
---
```
...
`/reindex` appends a bullet-point run summary to `.claude/logs/reindex.md`:
...
Append-only. One heading per run. Tight.
